...
for (const id of WUL.stationOrder) {
  ...
  (s.build || []).forEach((b, i) => {
    ...
    if (b.type === 'steps') {
      ...
      (b.steps || []).forEach((st) => [...(st.show || []), ...(st.focus || []), ...(st.hide || [])].forEach((e) => { if (!els.has(e) && !els.has(e.replace(/-\d+$/, '')) && ![...els].some((x) => x.replace(/-\d+$/, '') === e)) W(`${w}: step shows "${e}", which the stage does not have`); }));
    }
